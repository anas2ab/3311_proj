...
extern EIF_TYPED_VALUE F1562_15540(EIF_REFERENCE);
...
RTOID (F1562_15540)
EIF_TYPED_VALUE F1562_15540 (EIF_REFERENCE Current)
{
	GTCX
...
#define Result RTOTRR
	...
	RTEAA(l_feature_name, 1561, Current, 0, 0, 21607);
	RTSA(dtype);
	RTSC;
	RTME(dtype, 0);
	RTGC;
	RTDBGEAA(1561, Current, 21607);
	RTIV(Current, RTAL);
	RTOTP;
	RTHOOK(1);
	...
	if (RTAL & CK_ENSURE) {
		...
		tb1 = (((FUNCTION_CAST(EIF_TYPED_VALUE, (EIF_REFERENCE)) RTVF(11521, "is_current_file_system", Result))(Result)).it_b);
		if (tb1) {
			RTCK;
		} else {
			RTCF;
...
#define Result RTOTRR
	...
	(FUNCTION_CAST(void, (EIF_REFERENCE)) RTWC(11520, Dtype(tr1)))(tr1);
	RTNHOOK(1,1);
	Result = (EIF_REFERENCE) RTCCL(tr1);
	if (RTAL & CK_ENSURE) {
		RTHOOK(2);
